...
(() => {
  ...
  // ─── Operator login gate ────────────────────────────────────────────
  // Anyone with role OPERATOR (or ADMIN, for convenience) can access CMCC.
  if (!api.token() || !api.user()) {
    location.replace('/login.html?next=/cmcc.html&role=OPERATOR');
    return;
  }
  const __me = api.user();
  if (!['OPERATOR','ADMIN'].includes(__me.role)) {
    // Wrong role — drop the existing session so login.html doesn't bounce
    // them back to their old dashboard, then ask for an operator OTP.
  ...
  function seed() {
    ...
    const types = [
      ...
      { type: 'HIGH_VOLUME',            sev: 'info',     icon: 'graph_up',      text: 'Order volume +40% vs hourly avg' },
    ];
    for (let i = 0; i < 24; i++) {
  ...
  function topbar(route) {
    ...
    return `
      <header class="cmd-top">
        ...
        <button class="tb-btn" onclick="location.href='/admin.html'"><span class="ico">open_in_new</span>Per-community</button>
  ...
  function overview(root) {
    ...
    document.getElementById('incidentBtn').onclick = () => createIncidentSheet();
    wireAlertActions();
  }
