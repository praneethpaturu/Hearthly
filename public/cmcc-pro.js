...
window.cmccPro = (() => {
  ...
  function openPlaybook(an) {
    const rb = RUNBOOKS[an.type] || RUNBOOKS.DEFAULT;
    let step = 0;
    ...
    host.innerHTML = `<div class="pb-card">
      <div class="pb-head">
        ...
        <div><div class="ttl">${rb.title}</div><div class="sub">Incident ${an.id} · ${an.community} · ${new Date(an.when).toLocaleString()}</div></div>
  ...
  function injectSurgeCard() {
    ...
    card.innerHTML = `
      ...
      <div style="display:grid; grid-template-columns: 1fr auto; gap: 14px; align-items: center;">
        <div>
          <div style="font-size: 14px; color: var(--ink-2); line-height: 1.6;">
            ☔ <b>Sunday 9 AM</b> — laundry surge predicted in <b>Bangalore</b> (rain forecast 70%+, 3 affected communities). Expected <b>+38% volume</b>.
          </div>
